...
function doglyphs(symbols)
	...
	for i, j in pairs(symbols) do
		if testcond(j[2],j[1]) then
			...
			if unit ~= nil and not isglyph(unit) then
				...
				local unitname = get_broaded_str(unit.strings[UNITNAME])
				local unittable = {}
				unittable.values = {}
				unittable.strings = {}
				unittable.fixed = j[1]
    ...
    for i, v in pairs(rules) do
		...
		if (string.sub(v[1],1,6) == "glyph_") then
			addoption({v[1], v[2], v[3]}, copyconds({}, conds[i]), ids[i], nil, nil, {"glyphrule", v[1]})
		elseif string.sub(v[1],1,5) == "text_" then
			addoption({v[1], v[2], v[3]}, copyconds({}, conds[i]), ids[i], nil, nil, {"glyphrule", "glyph_" .. string.sub(v[1],6)})
		elseif string.sub(v[1],1,6) == "event_" then
			addoption({v[1], v[2], v[3]}, copyconds({}, conds[i]), ids[i], nil, nil, {"glyphrule", "glyph_" .. string.sub(v[1],6)})
		elseif string.sub(v[1],1,6) == "node_" then
			addoption({v[1], v[2], v[3]}, copyconds({}, conds[i]), ids[i], nil, nil, {"glyphrule", "glyph_" .. string.sub(v[1],5)})
		else
			--]]
