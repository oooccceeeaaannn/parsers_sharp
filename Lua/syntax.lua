function addunit(id,undoing_,levelstart_)
	...
	if ((name == "text") and (unit.values[TYPE] == 0)) or ((unit.strings[UNITTYPE] == "node") and node_types[string.sub(unit.strings[UNITNAME], 6, -1)] == 0) or (not is_str_special_prefixed(name__)) then
		objectlist[name_] = 1
	...
	if (string.sub(name__, 1, 6) == "glyph_") and validglyph then
		...
		fullunitlist[string.sub(name__, 7)] = 1
	end
	...
	if (string.sub(name__, 1, 6) == "logic_" and logic_types[string.sub(name__, 7)] == 0) then
		objectlist[string.sub(name__, 7)] = 1
		fullunitlist[string.sub(name__, 7)] = 1
	end
							...
							if (findnoun(b) == false) and (b ~= matdata[1]) then
								...
								if (protect == nil) then
									...
									if (#mat == 0) then
										...
										if (is_str_broad_noun(matdata[1])) or (is_str_special_prefix(matdata[1])) or (matdata[1] == "level") then
