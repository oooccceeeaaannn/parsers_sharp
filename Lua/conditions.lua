							...
							if (e ~= "dir") then
								for c,d in ipairs(f) do
									if (pnot == false) then
										...
										if (ulist == false) and d ~= pname and diff_or_excluded(d, pname) and (alreadyfound[bcode] == nil) then
											alreadyfound[bcode] = 1
											ulist = true
										end
									end
					...
					if (surrounds[dirid] ~= nil) then
						for c,d in ipairs(surrounds[dirid]) do
							if (pnot == false) then
								...
								if d ~= pname and diff_or_excluded(d, pname) and (alreadyfound[bcode] == nil) then
									alreadyfound[bcode] = 1
									allfound = allfound + 1
								end
							end
				...
				if ((b ~= "level") and (b ~= "empty")) or ((b == "level") and (unitcount["level"] > 0)) then
					if (pnot == false) then
						...
						for c,d in pairs(unitlists) do
							...
							(findnoun(c,nlist.short) == false and (not is_str_special_prefixed(c)) and string.sub(pname,1,4) ~= "meta")
							or ((get_pref(c) == get_pref(pname)) and (is_str_special_prefixed(c)))
			...
			for c,d in ipairs(featureindex[fullname]) do
				...
				if (checkedconds[tostring(dconds)] == nil) then
					if (string.sub(drule[1], 1, 4) ~= "not ") and (drule[2] == "is") and (drule[3] == fullname) then
						if (drule[1] ~= "empty") and (drule[1] ~= "level") and (not is_str_broad_noun(drule[1])) and (string.sub(drule[1],1,4) ~= "meta") then -- the changed line
