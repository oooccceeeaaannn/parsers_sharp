				...
				for iid,unitid in ipairs(checkthese) do
					...
					if (alreadyused[tileid] == nil) and (unit.values[TYPE] ~= 5) and (unit.flags[DEAD] == false) then
						for i=1,2 do
							...
							local hm2 = codecheck(unitid,nox,noy,i,nil,wordunitresult, true) --count letters
...
function addoption(option,conds_,ids,visible,notrule,tags_,visualonly_)
	...
	if (#option == 3) then
		...
		if (string.sub(target, 1, 5) == "group") or (string.sub(effect, 1, 5) == "group" and ((not is_str_writelike_verb(verb)) or not foundtag)) or (string.sub(target, 1, 9) == "not group") or (string.sub(effect, 1, 9) == "not group") then
			groupcond = true
		end
		...
		elseif (is_str_broad_noun(effect) or is_str_notted_broad_noun(effect)) and (targetnot ~= "not ") and verb ~= "is" and verb ~= "become" and verb ~= "make" and verb ~= "has" and verb ~= "write"
				and verb ~= "inscribe" and verb ~= "log" and verb ~= "code" then
			for a,b in pairs(fullunitlist) do -- fullunitlist contains all units, is new
				local reale = effect
		...
		if (string.sub(rule[1], 1, 5) ~= "group") and (string.sub(rule[1], 1, 9) ~= "not group") and (rule[2] ~= "is") then
			...
			if (memberships[rule[3]] ~= nil) then
				for a,b in ipairs(memberships[rule[3]]) do
					...
					elseif (is_str_broad_noun(b[1]) or string.sub(b[1],1,4) == "meta") and (rule[2] ~= "become" and rule[2] ~= "has" and rule[2] ~= "make" and rule[2] ~= "write") then
						foundtag = true
					end
				...
				if (rule[1] == b) or can_refer(rule[1], b) then
					for c,g in ipairs(ids) do
						for a,d in ipairs(g) do
							local idunit = mmf.newObject(d)
							local start = rule[1]
							local findbroadtag = false
							if (tags ~= nil) then
								for c2,g2 in ipairs(tags) do
									if is_str_broad_noun(g2) then
										findbroadtag = true
										break
									end
								end
							...
							-- Tässä pitäisi testata myös Group!
							if ((not findbroadtag) and (idunit.strings[UNITNAME] == start)) or (idunit.strings[UNITNAME] == "text_" .. start) or ((idunit.strings[UNITNAME] == "glyph_" .. start) and (tags[1] == "glyphrule")) or ((idunit.strings[UNITNAME] == start) and (tags[1] == "glyphrule")) or ((start == "all") and (name ~= "glyph")) then
								--MF_alert("Matching objects - found")
								found = true
		...
		if (totalvariants < limiter) then
			...
			if (#words > 0) then
				...
				if (sharedtype >= 0) and (prevsharedtype >= 0) and (#words > 0) and (maxw == 1) and (prevmaxw == 1) and (currw == 0) then
					if ((sharedtype == 0) and (prevsharedtype == 0)) or ((sharedtype == 2) and (prevsharedtype == 2)) or ((sharedtype == 0) and (prevsharedtype == 2)) then
						done = true
						sents[step] = nil
